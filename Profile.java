...
public class Profile {
    ...
    /** Sets the user's fitness goal, replacing any previous goal. */
    public void setFitnessGoal(String goal) {
        String cleaned = requireText(goal, "Fitness goal");
        goals.clear();
        goals.add(cleaned);
    }
    ...
    public void setWorkoutDuration(int minutes) {
        if (minutes <= 0) {
            throw new IllegalArgumentException("Workout duration must be a positive number of minutes.");
        }
        duration = minutes;
    }
